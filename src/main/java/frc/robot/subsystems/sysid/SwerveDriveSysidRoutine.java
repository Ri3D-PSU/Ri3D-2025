  ...
  public SysIdRoutine createNewRoutine(
    ...
    return new SysIdRoutine(
        ...
        new SysIdRoutine.Mechanism(
            ...
            log -> {
              log.motor("frontLeft")
                  .voltage(appliedVoltage.mut_replace(fl.getDriveVoltage(), Volts))
                  .linearPosition(distance.mut_replace(fl.getPosition().distanceMeters, Meters))
                  .linearVelocity(
                      velocity.mut_replace(fl.getVelocityMetersPerSec(), MetersPerSecond));
              log.motor("frontRight")
                  .voltage(appliedVoltage.mut_replace(-fr.getDriveVoltage(), Volts))
                  .linearPosition(distance.mut_replace(fr.getPosition().distanceMeters, Meters))
                  .linearVelocity(
                      velocity.mut_replace(fr.getVelocityMetersPerSec(), MetersPerSecond));
              log.motor("backLeft")
                  .voltage(appliedVoltage.mut_replace(bl.getDriveVoltage(), Volts))
                  .linearPosition(distance.mut_replace(bl.getPosition().distanceMeters, Meters))
                  .linearVelocity(
                      velocity.mut_replace(bl.getVelocityMetersPerSec(), MetersPerSecond));
              log.motor("backRight")
                  .voltage(appliedVoltage.mut_replace(-br.getDriveVoltage(), Volts))
                  .linearPosition(distance.mut_replace(br.getPosition().distanceMeters, Meters))
                  .linearVelocity(
                      velocity.mut_replace(br.getVelocityMetersPerSec(), MetersPerSecond));
            },
            subsystem));
  }
}
