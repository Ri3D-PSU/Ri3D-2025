...
public class AHRS implements Sendable {
  ...
  public AHRS(SPI.Port spi_port_id, byte update_rate_hz) {
    ...
    if (m_simDevice != null) {
      ...
      if (RuntimeDetector.isLinux()
          && !RuntimeDetector.isAthena()
          && spi_port_id == SPI.Port.kMXP) {
  ...
  public AHRS(SerialPort.Port serial_port_id, SerialDataType data_type, byte update_rate_hz) {
    ...
    if (m_simDevice != null) {
      ...
      if (RuntimeDetector.isLinux()
          && !RuntimeDetector.isAthena()
          && serial_port_id == SerialPort.Port.kMXP) {
   ...
   *
   * <p>Each quaternion value (W,X,Y,Z) is expressed as a value ranging from -1 to 1. This total
   * range (2) can be associated with a unit circle, since each circle is comprised of 2 PI Radians.
   *
