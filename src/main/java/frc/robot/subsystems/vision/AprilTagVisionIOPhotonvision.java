...
public class AprilTagVisionIOPhotonvision implements AprilTagVisionIO {
    ...
    private final Transform3d ROBOTTOCAM = new Transform3d(new Translation3d(0.5, 0.0, 0.5), new Rotation3d(0,0,0)); // From docs, cam mounted facing forward, half a meter forward of center, half a meter up from center.
    ...
    public AprilTagVisionIOPhotonvision(VisionIOInputs inputs) {
        inputs.targetType = AprilTagVisionIO.TargetType.APRILTAG;
    }

    public void updateInputs(VisionIOInputs inputs) {
        inputs.ntPose = poseSub.get();
        inputs.unreadResults = camera.getAllUnreadResults();
        inputs.latestEstimatedPose = updatePoseEstimator(inputs.unreadResults);
        inputs.latestCamToTargetTranslation = getCamToTag(inputs.unreadResults);
    }
